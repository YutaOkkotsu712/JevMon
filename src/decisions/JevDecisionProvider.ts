...
export function buildJevPayload(input: DecisionInput, model = 'jev-latest') {
  ...
  for (const detail of ['reduced', 'minimal'] as const satisfies readonly Detail[]) {
    ...
    if (bytes <= PAYLOAD_BUDGET_BYTES) return { body, detail, state, bytes, overBudget: false };
  ...
  const tokens = estimatedTokens(smallest.body);
  return { ...smallest, overBudget: true, ...(tokens > JEV_TOKEN_LIMIT ? { overTokenLimit: tokens } : {}) };
}
...
export class JevDecisionProvider implements DecisionProvider {
  ...
  async chooseAction(input: DecisionInput, context?: { signal?: AbortSignal }): Promise<DecisionResult> {
    ...
    if (payload.overTokenLimit) this.emit('jev_payload_over_token_limit');
    else if (overBudget) this.emit('jev_payload_over_budget');
