...
pub fn ability_after_damage_hit(
    ...
    let defending_pkmn = defending_side.get_active();
    match defending_pkmn.ability {
        Abilities::MUMMY | Abilities::LINGERINGAROMA => {
            if choice.flags.contact {
                instructions
                    .instruction_list
                    .push(Instruction::ChangeAbility(ChangeAbilityInstruction {
                        side_ref: *side_ref,
                        ability_change: defending_pkmn.ability as i16 - attacking_pkmn.ability as i16,
                    }));
                attacking_pkmn.ability = defending_pkmn.ability;
            }
        }
        Abilities::PICKPOCKET => {
            if damage_dealt > 0
                ...
                && defending_pkmn.hp > 0
                && defending_pkmn.item == Items::NONE
                && attacking_pkmn.item_can_be_removed()
                && attacking_pkmn.item != Items::NONE
            {
                let stolen_item = attacking_pkmn.item;
                instructions.instruction_list.push(Instruction::ChangeItem(ChangeItemInstruction {
                    side_ref: *side_ref,
                ...
                attacking_pkmn.item = Items::NONE;
                defending_pkmn.item = stolen_item;
            }
        }
        Abilities::WANDERINGSPIRIT => {
            if choice.flags.contact {
                let old_attacker_ability = attacking_pkmn.ability;
                instructions.instruction_list.push(Instruction::ChangeAbility(ChangeAbilityInstruction {
                    side_ref: *side_ref,
                    ability_change: Abilities::WANDERINGSPIRIT as i16 - old_attacker_ability as i16,
                }));
                instructions.instruction_list.push(Instruction::ChangeAbility(ChangeAbilityInstruction {
                    side_ref: side_ref.get_other_side(),
                    ability_change: old_attacker_ability as i16 - Abilities::WANDERINGSPIRIT as i16,
                }));
                attacking_pkmn.ability = Abilities::WANDERINGSPIRIT;
                defending_pkmn.ability = old_attacker_ability;
            }
        }
        Abilities::GULPMISSILE => {
            if damage_dealt > 0
...
pub fn ability_modify_attack_being_used(
    ...
    match attacking_pkmn.ability {
        ...
        Abilities::PRANKSTER => {
            // Dark types are immune only to a Prankster-boosted move aimed at them. Screens, hazards, Substitute,
            // self boosts and field moves still work with a Dark type across the field; every Prankster status move
            // used to fail here, so the search thought our Light Screen or Spikes did nothing against Umbreon.
            let aimed_at_the_target = attacker_choice.target == MoveTarget::Opponent
                && attacker_choice.side_condition.is_none()
                && !matches!(
