...
use crate::instruction::{
    ...
    SetSleepTurnsInstruction, StateInstructions, SwitchInstruction, ToggleTrickRoomInstruction,
};
use crate::pokemon::PokemonName;
use crate::state::{
    pokemon_index_iter, LastUsedMove, PokemonBoostableStat, PokemonIndex, PokemonSideCondition,
...
pub fn choice_special_effect(
    ...
    match choice.move_id {
        ...
        // Revival Blessing brings one fainted teammate back to the bench at half its max HP. It was a move with no
        // effect here, so the search scored it as a wasted turn and never chose it. The teammate is picked rather than
        // branched on: the one with the highest stat total, the usual choice. Placeholders (max HP 1) are skipped.
        // The bench Pokémon is healed through the active slot, which Switch moves without any other effect.
        Choices::REVIVALBLESSING => {
            let active_index = attacking_side.active_index;
            let mut best: Option<(PokemonIndex, i32)> = None;
            for idx in pokemon_index_iter() {
                let p = &attacking_side.pokemon[idx];
                if idx == active_index || p.hp > 0 || p.maxhp <= 1 || p.id == PokemonName::NONE {
                    continue;
                }
                let total = p.maxhp as i32
                    + p.attack as i32
                    + p.defense as i32
                    + p.special_attack as i32
                    + p.special_defense as i32
                    + p.speed as i32;
            ...
            if let Some((idx, _)) = best {
                let heal_amount = attacking_side.pokemon[idx].maxhp / 2;
                instructions
                    .instruction_list
                ...
                attacking_side.pokemon[idx].hp = heal_amount;
            }
        }
        Choices::PAINSPLIT => {
            if !defending_side
        ...
        Choices::TRICK | Choices::SWITCHEROO => {
            ...
            // A Choice lock belongs to the old held item. Trick can remove that item after
            // item_before_move disabled the user's other moves; the new holder will lock into
            // the move it uses with the received item when it acts.
            state.re_enable_disabled_moves(attacking_side_ref, &mut instructions.instruction_list);
            state.re_enable_disabled_moves(&attacking_side_ref.get_other_side(), &mut instructions.instruction_list);
        }
        Choices::SUNNYDAY => {
