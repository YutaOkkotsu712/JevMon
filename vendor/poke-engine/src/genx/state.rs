...
impl Side {
    ...
    pub fn trapped(&self, opponent_active: &Pokemon) -> bool {
            ...
            .contains(&PokemonVolatileStatus::PARTIALLYTRAPPED)
        {
            ...
            && active_pkmn.ability != Abilities::SHADOWTAG
        {
            // A Shadow Tag holder is not held by another one.
            return true;
...
impl State {
    pub fn root_get_all_options(&self) -> (Vec<MoveChoice>, Vec<MoveChoice>) {
        ...
        if self.side_one.slow_uturn_move {
            s1_options.clear();
            // Outrage, Thrash, Petal Dance and Raging Fury hold their user to the same move, as Encore does. Only
            // switching was blocked before, so the search played Outrage and then another move into whatever came in.
            let encored = self
                .side_one
                .volatile_statuses
