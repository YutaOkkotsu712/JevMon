...
pub fn generate_instructions_from_move(
    ...
    // start multi-hit
    let hit_count;
    let mut loaded_dice_hit_count = false;
    ...
    if incoming_instructions.percentage != 0.0 {
        run_move_with_hit_count_branches(
            state,
            attacking_side,
            incoming_instructions,
            hit_count,
            loaded_dice_hit_count,
            does_damage,
            regular_damage,
            choice,
            defender_choice,
 ...
 * paths from the same pre-hit state so the search sees a possible fifth hit and KO. */
fn run_move_with_hit_count_branches(
    state: &mut State,
    attacking_side: SideReference,
    instructions: StateInstructions,
    hit_count: i8,
    loaded_dice_hit_count: bool,
    does_damage: bool,
    damage_amount: i16,
    choice: &mut Choice,
    defender_choice: &Choice,
    ...
    if loaded_dice_hit_count {
        let mut four = instructions.clone();
        four.update_percentage(0.5);
        run_move(state, attacking_side, four, 4, does_damage, damage_amount, choice, defender_choice, final_instructions);
        state.apply_instructions(&instructions.instruction_list);
        let mut five = instructions;
        five.update_percentage(0.5);
        run_move(state, attacking_side, five, 5, does_damage, damage_amount, choice, defender_choice, final_instructions);
    } else {
        run_move(state, attacking_side, instructions, hit_count, does_damage, damage_amount, choice, defender_choice, final_instructions);
    }
}
...
// Soul-Heart responds to any faint while its holder is active, including recoil,
// contact damage and residual damage. Replaying the instructions lets us place
// the boost immediately after the faint, before the other Pokemon takes its turn.
fn apply_soul_heart_after_faints(state: &mut State, instructions: &mut StateInstructions) {
    let can_have_soul_heart = state.side_one.pokemon.pkmn.iter().chain(state.side_two.pokemon.pkmn.iter())
        .any(|pokemon| pokemon.ability == Abilities::SOULHEART);
    if !can_have_soul_heart {
        state.apply_instructions(&instructions.instruction_list);
        return;
    }

    let original = std::mem::take(&mut instructions.instruction_list);
    let mut completed = Vec::with_capacity(original.len() + 2);
    for (index, instruction) in original.iter().enumerate() {
        let before = [state.side_one.get_active_immutable().hp, state.side_two.get_active_immutable().hp];
        state.apply_one_instruction(instruction);
        completed.push(instruction.clone());
        let fainted = (before[0] > 0 && state.side_one.get_active_immutable().hp == 0)
            || (before[1] > 0 && state.side_two.get_active_immutable().hp == 0);
        if !fainted {
            continue;
        }
        let gas_active = [state.side_one.get_active_immutable(), state.side_two.get_active_immutable()]
            .iter().any(|pokemon| pokemon.hp > 0 && pokemon.ability == Abilities::NEUTRALIZINGGAS);
        if gas_active {
            continue;
        }
        for side_ref in [SideReference::SideOne, SideReference::SideTwo] {
            let holder = state.get_side_immutable(&side_ref).get_active_immutable();
            if holder.hp == 0 || holder.ability != Abilities::SOULHEART {
                continue;
            }
            // This prefix may have been processed after the first move already.
            if matches!(original.get(index + 1), Some(Instruction::Boost(boost))
                if boost.side_ref == side_ref && boost.stat == PokemonBoostableStat::SpecialAttack)
            {
                continue;
            }
            let mut gained = StateInstructions::default();
            apply_boost_instruction(
            ...
            completed.extend(gained.instruction_list);
        }
    }
    instructions.instruction_list = completed;
}
...
fn after_move_finish(state: &mut State, final_instructions: &mut Vec<StateInstructions>) {
    for state_instructions in final_instructions.iter_mut() {
        apply_soul_heart_after_faints(state, state_instructions);
...
pub fn generate_instructions_from_move_pair(
    ...
    // End-of-turn damage is appended after the move passes above. Process its
    // faints as well, without duplicating boosts already inserted after a move.
    if state.side_one.pokemon.pkmn.iter().chain(state.side_two.pokemon.pkmn.iter())
        .any(|pokemon| pokemon.ability == Abilities::SOULHEART)
    {
        for state_instruction in state_instructions_vec.iter_mut() {
            apply_soul_heart_after_faints(state, state_instruction);
            state.reverse_instructions(&state_instruction.instruction_list);
        }
    }
    state_instructions_vec
