...
impl Node {
    ...
    pub unsafe fn expand(
        ...
        // Damage rolls branch at every depth. When only the root and its children branched, deeper turns used the
        // average roll, so a knockout that half the rolls land became certain one ply later: sacrificing a 4%
        // Oinkologne to Typhlosion scored 0.94, because the sacrifice pushed Toucannon's coin-flip Beak Blast past
        // the depth where rolls were branched. Chance children are sampled by probability, so this costs breadth
        // at expansion, not visits.
        let should_branch_on_damage = true;
