...
use crate::engine::evaluate::{evaluate, evaluate_features};
...
/// The evaluation's unweighted terms for a state, side one minus side two, one `name value` line each, then the
/// evaluation under the weights in use: what the weights are fitted against.
#[derive(Parser)]
struct Features {
...
pub fn main() {
    ...
    match args.subcmd {
        ...
        Some(subcmd) => match subcmd {
            SubCommand::Features(features) => {
                let state = State::deserialize(features.state.as_str());
                for (name, value) in evaluate_features(&state).values() {
                    println!("{} {}", name, value);
                }
