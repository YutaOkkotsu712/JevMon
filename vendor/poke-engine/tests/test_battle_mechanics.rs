...
#[test]
fn test_soulheart_boosts_on_its_own_knockout() {
    let mut state = State::default();
    state.side_one.get_active().ability = Abilities::SOULHEART;
    state.side_two.get_active().hp = 1;
    ...
    assert!(instructions[0].instruction_list.contains(&Instruction::Boost(BoostInstruction {
        side_ref: SideReference::SideOne,
        stat: PokemonBoostableStat::SpecialAttack,
        amount: 1,
    })));
    assert_eq!(instructions[0].instruction_list.iter().filter(|instruction| matches!(instruction,
        Instruction::Boost(boost) if boost.stat == PokemonBoostableStat::SpecialAttack)).count(), 1);
}

#[test]
fn test_soulheart_boosts_when_the_other_side_faints_from_poison() {
    let mut state = State::default();
    state.side_one.get_active().hp = 1;
    state.side_one.get_active().status = PokemonStatus::POISON;
    state.side_two.get_active().ability = Abilities::SOULHEART;
    ...
    assert!(instructions.iter().all(|branch| branch.instruction_list.contains(&Instruction::Boost(BoostInstruction {
        side_ref: SideReference::SideTwo,
        stat: PokemonBoostableStat::SpecialAttack,
        amount: 1,
    }))));
...
#[test]
fn test_soulheart_boosts_when_the_other_side_faints_from_life_orb() {
    let mut state = State::default();
    state.side_one.get_active().hp = 1;
...
#[test]
fn test_lingering_aroma_and_wandering_spirit_contact_changes() {
    let mut aroma = State::default();
    aroma.side_one.get_active().ability = Abilities::INTIMIDATE;
    aroma.side_two.get_active().ability = Abilities::LINGERINGAROMA;
    let aroma_result = set_moves_on_pkmn_and_call_generate_instructions(&mut aroma, Choices::TACKLE, Choices::SPLASH);
    assert!(aroma_result[0].instruction_list.contains(&Instruction::ChangeAbility(ChangeAbilityInstruction {
        side_ref: SideReference::SideOne,
        ability_change: Abilities::LINGERINGAROMA as i16 - Abilities::INTIMIDATE as i16,
    })));

    let mut spirit = State::default();
    spirit.side_one.get_active().ability = Abilities::INTIMIDATE;
    spirit.side_two.get_active().ability = Abilities::WANDERINGSPIRIT;
    let spirit_result = set_moves_on_pkmn_and_call_generate_instructions(&mut spirit, Choices::TACKLE, Choices::SPLASH);
    assert!(spirit_result[0].instruction_list.contains(&Instruction::ChangeAbility(ChangeAbilityInstruction {
        side_ref: SideReference::SideOne,
        ability_change: Abilities::WANDERINGSPIRIT as i16 - Abilities::INTIMIDATE as i16,
    })));
    assert!(spirit_result[0].instruction_list.contains(&Instruction::ChangeAbility(ChangeAbilityInstruction {
        side_ref: SideReference::SideTwo,
        ability_change: Abilities::INTIMIDATE as i16 - Abilities::WANDERINGSPIRIT as i16,
    })));
}

#[test]
fn test_pickpocket_steals_only_when_it_is_hit_by_contact() {
    let mut state = State::default();
    state.side_one.get_active().item = Items::LEFTOVERS;
    state.side_two.get_active().ability = Abilities::PICKPOCKET;
    let hit = set_moves_on_pkmn_and_call_generate_instructions(&mut state, Choices::TACKLE, Choices::SPLASH);
    assert!(hit[0].instruction_list.contains(&Instruction::ChangeItem(ChangeItemInstruction {
        side_ref: SideReference::SideOne,
        current_item: Items::LEFTOVERS,
        new_item: Items::NONE,
    })));
    ...
    let no_contact = set_moves_on_pkmn_and_call_generate_instructions(&mut state, Choices::EARTHQUAKE, Choices::SPLASH);
    assert!(no_contact[0].instruction_list.iter().all(|instruction| !matches!(instruction, Instruction::ChangeItem(_))));

    let mut attacker = State::default();
    attacker.side_one.get_active().ability = Abilities::PICKPOCKET;
    attacker.side_two.get_active().item = Items::LEFTOVERS;
    let no_steal = set_moves_on_pkmn_and_call_generate_instructions(&mut attacker, Choices::TACKLE, Choices::SPLASH);
    assert!(no_steal[0].instruction_list.iter().all(|instruction| !matches!(instruction, Instruction::ChangeItem(_))));
}
...
#[test]
fn test_tricking_away_choice_scarf_releases_user_and_locks_recipient_move() {
    let mut state = State::default();
    state.side_one.get_active().item = Items::CHOICESCARF;
    state.side_one.get_active().replace_move(PokemonMoveIndex::M1, Choices::THUNDERBOLT);
    state.side_two.get_active().item = Items::LEFTOVERS;
    state.side_two.get_active().replace_move(PokemonMoveIndex::M1, Choices::TACKLE);
    let instructions = set_moves_on_pkmn_and_call_generate_instructions(
        &mut state, Choices::TRICK, Choices::CALMMIND,
    );
    assert_eq!(instructions.len(), 1);
    state.apply_instructions(&instructions[0].instruction_list);
    assert_eq!(state.side_one.get_active().item, Items::LEFTOVERS);
    assert!(!state.side_one.get_active().moves.m1.disabled, "Trick frees the user's other moves");
    assert_eq!(state.side_two.get_active().item, Items::CHOICESCARF);
    assert!(state.side_two.get_active().moves.m1.disabled, "Calm Mind is the recipient's Choice-locked move");
}

#[test]
fn test_loaded_dice_branches_four_and_five_hits() {
    let mut state = State::default();
    state.side_one.get_active().item = Items::LOADEDDICE;
    state.side_two.get_active().maxhp = 500;
    state.side_two.get_active().hp = 500;
    let outcomes = set_moves_on_pkmn_and_call_generate_instructions(
        &mut state, Choices::ICICLESPEAR, Choices::SPLASH,
    );
    assert_eq!(outcomes.len(), 2, "Loaded Dice must keep both hit counts");
    let mut damage = outcomes.iter().map(|outcome| {
        assert_eq!(outcome.percentage, 50.0);
        outcome.instruction_list.iter().filter_map(|ins| match ins {
            Instruction::Damage(d) if d.side_ref == SideReference::SideTwo => Some(d.damage_amount),
            _ => None,
        }).sum::<i16>()
    }).collect::<Vec<_>>();
    damage.sort();
    assert!(damage[0] > 0 && damage[1] > damage[0], "the fifth hit must deal more damage");

    state.side_one.get_active().ability = Abilities::SKILLLINK;
    let skill_link = set_moves_on_pkmn_and_call_generate_instructions(
        &mut state, Choices::ICICLESPEAR, Choices::SPLASH,
    );
    assert_eq!(skill_link.len(), 1, "Skill Link fixes the count at five even with Loaded Dice");
}
...
fn test_prankster_screen_and_hazard_still_work_with_a_dark_type_across() {
    let mut state = State::default();
    state.side_one.get_active().ability = Abilities::PRANKSTER;
    state.side_two.get_active().types.0 = PokemonType::DARK;
    for (mv, side_ref, side_condition) in [
        (Choices::LIGHTSCREEN, SideReference::SideOne, PokemonSideCondition::LightScreen),
        (Choices::STEALTHROCK, SideReference::SideTwo, PokemonSideCondition::Stealthrock),
    ] {
        let vec_of_instructions =
            set_moves_on_pkmn_and_call_generate_instructions(&mut state, mv, Choices::SPLASH);
        assert!(
            vec_of_instructions[0].instruction_list.iter().any(|i| matches!(i,
                Instruction::ChangeSideCondition(c) if c.side_ref == side_ref && c.side_condition == side_condition)),
            "{:?} must still work against a Dark type",
            mv
        );
    }
}
...
#[test]
fn test_lockedmove_holds_the_user_to_the_same_move() {
    let mut state = State::default();
    state
        .side_one
        .volatile_statuses
    ...
    assert!(side_one_options.iter().all(|c| matches!(
        c,
        MoveChoice::Move(PokemonMoveIndex::M1) | MoveChoice::MoveTera(PokemonMoveIndex::M1)
    )));
}

#[test]
fn test_revival_blessing_revives_the_strongest_fainted_teammate_at_half_hp() {
    let mut state = State::default();
    state.side_one.pokemon[PokemonIndex::P2].id = PokemonName::PIKACHU;
    state.side_one.pokemon[PokemonIndex::P2].hp = 0;
    state.side_one.pokemon[PokemonIndex::P2].maxhp = 200;
    state.side_one.pokemon[PokemonIndex::P4].id = PokemonName::PIKACHU;
    state.side_one.pokemon[PokemonIndex::P4].hp = 0;
    state.side_one.pokemon[PokemonIndex::P4].maxhp = 150;
    let vec_of_instructions = set_moves_on_pkmn_and_call_generate_instructions(
        &mut state,
    ...
    let expected_instructions = vec![StateInstructions {
        ...
        instruction_list: vec![
            ...
            Instruction::Switch(SwitchInstruction {
                side_ref: SideReference::SideOne,
                previous_index: PokemonIndex::P2,
...
#[test]
fn test_revival_blessing_does_nothing_without_a_fainted_teammate_or_with_only_placeholders() {
    let mut state = State::default();
    state.side_one.pokemon[PokemonIndex::P3].hp = 0;
    state.side_one.pokemon[PokemonIndex::P3].maxhp = 1;
    let vec_of_instructions = set_moves_on_pkmn_and_call_generate_instructions(
        &mut state,
        Choices::REVIVALBLESSING,
        Choices::SPLASH,
